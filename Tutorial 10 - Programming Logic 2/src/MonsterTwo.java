...
//		int maxYBoardSpace = battleBoard.length - 1;
		...
		int randMoveDirection = (int) (Math.random() * 4); // 3-0+1 (int olduðundan)
...
//		move distance 0 ise move direction'ýn önemi kalmaz
...
//		amacýmýz monsterý hareket ettirmek
...
//			y bu projede düþey yön için kullanýlýyor 
//			ve monsterýn ilk düþey konumunu belirtiyor.
			if((this.yPosition - randMoveDistance) < 0)
			{	this.yPosition = 0;
//			off board olunca hareket ettirmek için continue kullanýlabilirdi
			} else {
				this.yPosition = this.yPosition - randMoveDistance;
			}
			
//			check if the monster falls off the board in horizontal eastwards direction
//			(x bu projede yatay yön için ölçüm yapýyor)
...
//		amacýmýz hareket eden monsterýn diðerleri ile çakýþmasýný önlemek
...
//				throws out of for loop but not the while loop
		...
		// ilk sta execute edildiðinden do while loop kullanmak daha doðru olur:
		//		do {
		//			...
		//		} while(isSpaceOpen);
...
//		for loop ile variable a ulaþmak yerine 

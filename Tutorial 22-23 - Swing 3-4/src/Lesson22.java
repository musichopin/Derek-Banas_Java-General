...
//imported in order to handle the even handling
...
//		(unlike checkboxes one of them can be selected at a time)
		...
		// Create a new panel to hold radio buttons (radio button panel)
		...
		// Add the radio buttons to the panel 1 by 1 (not by button group)
...
//		(major tick uzun olur ve sayýlar yazýlýr)
		...
		public void actionPerformed(ActionEvent e){
			...
			if(e.getSource() == button1){
				...
				try{
					...
					// JOptionPane.showMessageDialog() is the method that opens the popup on the screen,
					// It’s what pops up on the screen when I click on the button.
					// (parentComponent (what frame is tied to), message, title, error icon)
					// Error Icons: WARNING_MESSAGE, QUESTION_MESSAGE, PLAIN_MESSAGE, ERROR_MESSAGE
					
//					For showMessageDialog the first attribute is for the frame.
//					To refer to the class that encloses the ListenForButton class you type the className.this. 
//					we give reference to the actual frame with "Lesson24.this" 
//					(inner class olmasý ve inner class içerisinde frame'e ait instance'ý çaðýrmamýz ile ilgili) 
				...
				if(dollarSign.isSelected()) {
					...
					// Defines that you want to format a number with $ (rather local currency) and commas
					
					NumberFormat numFormat = NumberFormat.getCurrencyInstance();
//					getCurrencyInstance() actually returns a NumberFormat object. That is why you don’t need new.
...
//		*Below methods could and should have been condensed into one method*
//		metodlarýn statik olup olmamasý önemli deðil,
//		(zira bu metodlarýn çaðýrýldýðý inner class'daki actionPerformed metodu statik deðil)

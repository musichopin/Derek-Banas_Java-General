...
// base class for creating all swing components even custom ones

import javax.swing.JFrame;
// we draw everything in jframe

import java.awt.*;
// allows us to create interfaces and paint graphics

import java.awt.geom.*;
// defines two dimensional shapes

@SuppressWarnings("serial")
// we can assign serial numbers to multiple classes,
// we shut that off
...
//		Java calls the paint method whenever needs to repaint the draw, ***
		this.add(new DrawStuff(), BorderLayout.CENTER); // border layout
//		(border layout haricinde, flow layout, box layout, GridBagLayout vardýr)
//		we add new component and it is set in the center
		
		this.setVisible(true);
		
	}
	
	// Creating my own custom component by extending JComponent
	// JComponent is the base class for all swing components. Even custom ones
	
	private class DrawStuff extends JComponent{  // inner class
		
		// Graphics is the base class that allows for drawing on components
		// Graphics2D extends graphics so we can draw dimensional shapes and images. 
		//	We are casting graphics (g) into a Graphics2D object
		public void paint(Graphics g){
//			paint metodu component üstüne çizim yapmak için ayrýlmýþ özel metod
...
//			and we don't need to do that.*
			...
			Graphics2D graph2 = (Graphics2D)g; // downcasting
...
//			(x saða doðru y ise aþaðý doðru artar)
			
			Shape drawLine = new Line2D.Float(20, 90, 55, 250);
			
			// Start x (saða artar), start y (aþaðý artar), width, height, start angle degrees (sol kýsým), 
			// angular extent (yarým daire, çeyrek daire vs.), OPEN, CHORD, PIE
			...
			Shape drawEllipse = new Ellipse2D.Float(200, 200, 100, 100);
			
			// Round off the rectangle by defining arc height then arc width
			
			Shape drawRoundRec = new RoundRectangle2D.Double(250, 25, 50, 50, 45, 45);
...
//			?
			...
			// Draw a shape with a fill (içini doldurur)
			
			graph2.fill(drawRoundRec);
			
			graph2.fill(drawRect); // fills inside the frame
			
			// changes the stroke color
			graph2.setPaint(Color.BLACK);
			
			graph2.draw(cubicCurve); // eðri çizer
			
			graph2.draw(drawRect); // draws the frame
...
//			graph2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0F));
			...
			// VERTICAL GRADIENT
//			end x veya end y 0 olursa o yönde deðiþim gözlenmez demektir (yani ikinci renke geçiþ olmaz)
